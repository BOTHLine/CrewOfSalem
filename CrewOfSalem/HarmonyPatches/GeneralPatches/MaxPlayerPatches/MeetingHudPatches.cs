...
namespace CrowdedMod.Patches
{
    internal static class MeetingHudPatches
    {
        ...
        private static int    MAXPages => Mathf.CeilToInt(GameData.Instance.AllPlayers.Count / 10f);

        //[HarmonyPatch(typeof(MeetingHud), nameof(MeetingHud.Update))]
        private static class VoteGuiPatch
        {
        ...
        private static class MeetingHudCheckForEndVotingPatch
        {
            public static bool Prefix(MeetingHud __instance)
            {
                ...
                if (AmongUsClient.Instance.AmClient)
                {
                    MessageWriter messageWriter =
                        AmongUsClient.Instance.StartRpc(__instance.NetId, 23, SendOption.Reliable);
            ...
            private static byte[] CalculateVotes(PlayerVoteArea[] states)
            {
                ...
                foreach (PlayerVoteArea playerVoteArea in states)
                {
        ...
        private static class MeetingHudHandleRpcPatch
        {
            private static bool Prefix(MeetingHud __instance, [HarmonyArgument(0)] byte callId,
            ...
            public static void VotingComplete(MeetingHud __instance, byte[] states, byte[] votes,
                        ...
                        if ((states[j] & 128) == 0)
                        {
                            ...
                            if (playerVoteArea.TargetPlayerId == votedFor)
                            {
                                ...
                                transform.localPosition = __instance.CounterOrigin +
                                                          new Vector3(__instance.CounterOffsets.x * num2,
                                                              0f, 0f);
                                __instance.StartCoroutine(
                                    Effects.Bloop(num2 * 0.5f, transform, 1f, 0.5f));
                                num2++;
                            } else if (i == 0 && votedFor == -1)
                            {
                                ...
                                transform.localPosition = __instance.CounterOrigin +
                                                          new Vector3(__instance.CounterOffsets.x * num,
                                                              0f, 0f);
                                __instance.StartCoroutine(Effects.Bloop(num * 0.5f, transform, 1f,
                                    0.5f));
                                num++;
                            }
                        }
